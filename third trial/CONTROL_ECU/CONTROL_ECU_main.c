 ...
 * THE CODE FLOW
 *
 * I enter the main and initialize the UART & the motor & buzzer & TWI
 *
 * this controller is mainly controlling the actuators and
 * it's main function is to compare the password the user send with the one in eeprom
 *
 * I go to the eeprom to check if the system already has set a password or not
 * if true dont call set_change password and send a byte for controller 1 to jump to the main
 * if false then send a byte for controller 1 to enter change password function and here
 * we wait for 2 passwords in set_chage password function
 *
 * the while(1) has 4 main states
 *
 * -open door
 * -change pass
 * -rotate motor
 * -activate buzzer
 *
 * each state is designed to meet the first controller needs
 *
 * so in open door mode
 * the controller is waiting for the user to send 1 password and compare it then
 * respond with either a match or mismatch
 *
 * change password mode the controller is waiting for 1 password which is the current password
 * then 2 passwords which are the new passwords
 * he have to make sure that every password is matched or else it will send mismatch to the first controller
 *
 * rotate motor will rotate the motor using the timer
 *
